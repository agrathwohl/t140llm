...
Runnable demos live in [`examples/`](../examples/). Build the library first (`npm run build`), then run any demo with `node examples/<file>`.

## Transport comparison — `demo.js`

Sends the same text stream over WebSocket, RTP, SRTP, and a direct Unix socket, with a timing comparison against raw transmission. Run [`receiver.js`](../examples/receiver.js) in a second terminal to see the far end.
...
Combines several LLM streams into one RTP output, tagging each with a CSRC stream ID. Pair it with [`demultiplexer_example.js`](../examples/demultiplexer_example.js) to split them back apart on the far end.
...
Hides RTP packets inside cover media, including LLM-generated steganography algorithms and custom steganographic transports. See [`steganography/README.md`](../examples/steganography/README.md).
